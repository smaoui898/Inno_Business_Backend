...
public class UnassignBeneficiairesFromShiftUseCaseImpl implements UnassignBeneficiairesFromShiftUseCase {
    ...
    public UnassignBeneficiairesFromShiftUseCaseImpl(UserRepositoryPort userRepository,ShiftRepositoryPort shiftRepository,BeneficiaireRepositoryPort beneficiaireRepository) {
    ...
    @Override
    public UnassignResult execute(String ownerEmail, UUID shiftId) {
        ...
        // Compte avant désaffectation pour retourner le nombre
        long count = beneficiaireRepository.countByShiftId(shiftId);
        // ye7seb nombre beneficiaire a désaffecter
        beneficiaireRepository.unassignAllFromShift(shiftId);
        // yeraj3 number beneficiaire a désaffecter
        return new UnassignResult(count);
    }
